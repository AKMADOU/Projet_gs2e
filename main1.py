...
def cv_similarity(resume_folder: str, job_path: str, keywords: str) -> List[CVMatch]:
    threshold = 45.0  # Seuil de similarité fixe
...
def move_files(matches: List[CVMatch], target_directory: str):
...
def get_cv_matches(resume_folder: str = Query(..., description="Chemin du dossier contenant les CV"),
                   ...
                   keywords: str = Query(..., description="Liste de mots-clés pour le filtrage initial (séparés par des virgules)")) -> List[CVMatch]:
    matches_above_threshold = cv_similarity(resume_folder, job_path, keywords)
    ...
    move_files(matches_above_threshold, tri_cv_folder)
